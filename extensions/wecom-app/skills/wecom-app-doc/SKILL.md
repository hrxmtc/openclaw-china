---
name: wecom-app-doc
description: 企业微信自建应用（wecom-app）文档与智能表格 MCP 操作。当用户在 wecom-app 场景下提到企业微信文档、智能表格、创建文档、编辑文档、新建文档、写文档时激活。优先复用 wecom-app 插件写入的文档 MCP 配置，并通过 mcporter 调用文档能力。
metadata:
  {
    "openclaw":
      {
...
- 用户在 `wecom-app` 会话里要创建企业微信文档
- 用户在 `wecom-app` 会话里要创建企业微信智能表格
...
```bash
~/.openclaw/wecomAppConfig/config.json
```
...
如果本地没有 `mcpConfig.doc`，说明当前 `wecom-app` 还没配置文档 MCP。此时：

- 优先让用户提供 `StreamableHttp URL` 或 `JSON Config`
- 或指导用户在 OpenClaw 配置里设置：
  - `channels.wecom-app.docs.mcp.type`
  - `channels.wecom-app.docs.mcp.url`
- 多账号场景可设置：
  - `channels.wecom-app.accounts.<accountId>.docs.mcp.type`
  - `channels.wecom-app.accounts.<accountId>.docs.mcp.url`
...
- 如果返回 `850001`，说明还需要用户提供配置，按上面的配置引导继续处理
